...
class TestApiKeyPanel:
    ...
    def test_entered_key_never_reaches_os_environ(self):
        ...
        widget = _input_by_key(at, "user_key_sarvam_api_key")
        widget.set_value(SECRET).run()

        assert not at.exception
        assert os.environ.get("SARVAM_API_KEY") != SECRET
    ...
    def test_entered_key_is_actually_applied_to_settings(self, monkeypatch):
        ...
        import voice_agent.health as health

        def fake_check(settings, only=None):
            captured["probe"] = settings.sarvam_api_key
            return []
        ...
        at = AppTest.from_file(str(APP), default_timeout=90)
        at.run()
        _input_by_key(at, "user_key_sarvam_api_key").set_value(SECRET).run()
    ...
    def test_blank_input_does_not_override_configured_key(self, monkeypatch):
        monkeypatch.setenv("SARVAM_API_KEY", "key-from-environment")
