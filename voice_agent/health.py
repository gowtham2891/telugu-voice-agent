...
def missing_credentials(settings) -> List[MissingCredential]:
    ...
    # STT and TTS share one Sarvam key, so report it once.
    if "sarvam" in {settings.stt, settings.tts} and not settings.sarvam_api_key.strip():
        stages = [name for name, value in (("speech-to-text", settings.stt),
                                           ("text-to-speech", settings.tts))
                  if value == "sarvam"]
        missing.append(MissingCredential(
            "Sarvam AI key", "SARVAM_API_KEY",
            " and ".join(stages), "sarvam.ai"))

    if settings.llm == "openai" and not settings.openai_api_key.strip():
        missing.append(MissingCredential(
            "OpenAI API key", "OPENAI_API_KEY",
            "the 'openai' language model", "platform.openai.com/api-keys"))

    if settings.llm == "gemini" and not settings.gemini_api_key.strip():
        missing.append(MissingCredential(
            "Gemini API key", "GEMINI_API_KEY",
            "the 'gemini' language model", "aistudio.google.com/apikey"))
