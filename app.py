...
from voice_agent import VoiceAgent, __version__
from voice_agent.config import ConfigError, Settings
from voice_agent.health import check_settings, missing_credentials
...
KEY_FIELDS = [
    (
        "sarvam_api_key",
        "Sarvam AI key",
        "Required when STT or TTS is `sarvam`. Covers both. sarvam.ai.",
    ),
    (
        "openai_api_key",
        "OpenAI API key",
        "Required when the language model is `openai`.",
    ),
    (
        "gemini_api_key",
        "Gemini API key",
        "Required when the language model is `gemini`.",
    ),
]
...
def main() -> None:
    ...
    with chat_tab:
        ...
        if message and not credential_gate(settings):
            message = None
